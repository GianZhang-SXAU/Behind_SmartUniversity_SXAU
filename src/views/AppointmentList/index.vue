<template>
  <div class="listTemplate" ref="contentRef">
    ...
    <a-modal v-model:open="appointmentOpen" title="审核" @ok="AppointmentOk">
      <a-form
        :model="appointmentFormState"
        :label-col="{ span: 6 }"
        :wrapper-col="{ span: 16 }"
        autocomplete="off"
      >
        <a-form-item label="审核">
          <a-select ref="select" v-model:value="appointmentFormState.shenhe">
    ...
    >
      <a-form class="formMessage" layout="inline" :model="formMessage">
        <a-row :gutter="24">
          ...
          <a-col :span="12">
            <a-form-item label="审核id" name="shenheid">
...
const appointmentFormState = reactive({});
const AppointmentOpen = (row) => {
  appointmentOpen.value = true;
  Object.assign(appointmentFormState, row);
};
const AppointmentOk = async () => {
  const { id, shenheid, shenhe } = appointmentFormState;
  const { code } = await POST("/appointment/update", {
    id,
    shenhe,
  });
...
const DelRow = async ({ id }) => {
  const { code } = await POST("/appointment/delete", [id]);
...
const MessageRow = async ({ id }) => {
  const { code, data } = await POST("/appointment/info", { id });
  if (code != 0) return;
...
const OnSubmit = async (values) => {
  ...
  const { code } = await POST(
    `/appointment/${id != undefined ? "update" : "save"}`,
    data
  );
