  ...
  <div class="listTemplate" ref="contentRef">
    <div class="topChunk" ref="topChunk">
      <a-form
        ...
        @finish="FinishSearch"
      >
        <a-form-item class="formSearch-item" label="线路名" name="name">
          <a-input v-model:value="formSearch.name" placeholder="请输入线路名" />
        </a-form-item>
    ...
    >
      ...
      <template #expandedRowRender="{ record }">
        <a-descriptions bordered>
          <a-descriptions-item label="简介" :span="3" v-if="record.detail">
            <p style="margin: 0" v-html="record.detail"></p>
          </a-descriptions-item>

          <a-descriptions-item label="详情" :span="3" v-if="record.details">
    ...
    >
      <a-form class="formMessage" layout="inline" :model="formMessage">
        <a-row :gutter="24">
          ...
          <a-col :span="12">
            <a-form-item label="线路名" name="name">
              <p v-html="formMessage.name"></p>
            </a-form-item>
          </a-col>

          <a-col :span="12">
            <a-form-item label="线路编号" name="routenumbers">
              <p v-html="formMessage.routenumbers"></p>
            </a-form-item>
          </a-col>

          <a-col :span="12">
            <a-form-item label="起始站" name="initiatingstation">
              <p v-html="formMessage.initiatingstation"></p>
            </a-form-item>
          </a-col>

          <a-col :span="12">
            <a-form-item label="终点站" name="terminal">
              <p v-html="formMessage.terminal"></p>
            </a-form-item>
          </a-col>

          <a-col :span="12">
            <a-form-item label="总长度" name="totallength">
              <p v-html="formMessage.totallength"></p>
            </a-form-item>
          </a-col>
...
const AddRouteplanning = () => {
  ClearReactiveObj(formData);
  createModalFlag.value = true;
  createModalTitle.value = "新增线路规划";
};
...
const DelRow = async ({ id }) => {
  const { code } = await POST("/routeplanning/delete", [id]);
...
const MessageRow = async ({ id }) => {
  const { code, data } = await POST("/routeplanning/info", { id });
  if (code != 0) return;
...
const OnSubmit = async (values) => {
  ...
  const { code } = await POST(
    `/routeplanning/${id != undefined ? "update" : "save"}`,
    data
  );
  if (code != 0) return message.error("创建失败");
  router.push("/RouteplanningList");
};
// 重置表单
const OnReset = () => {
